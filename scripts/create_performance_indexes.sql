...
-- 3. Leave Balance Table Indexes
-- For fetching leave balances (employee dashboard)
CREATE INDEX IF NOT EXISTS idx_leave_balance_user_id ON leave_balance(user_id);

-- For leave balance updates
CREATE INDEX IF NOT EXISTS idx_leave_balance_user_updated ON leave_balance(user_id, updated_at DESC);

-- 4. Holiday Calendar Indexes
-- For holiday lookups by country and year
CREATE INDEX IF NOT EXISTS idx_holiday_calendar_country_year ON holiday_calendar(country, year);
